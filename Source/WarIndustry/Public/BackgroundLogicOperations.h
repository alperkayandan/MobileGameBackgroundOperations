...
class WARINDUSTRY_API UBackgroundLogicOperations : public UObject
{
	...
	public:
		...
		UFUNCTION(BlueprintCallable, Category = "AsyncBackgroundLogic")
		void CheckCountryBordersForOffer(UObject* WorldContextObject);

	private:

		void ControlGlobalFactorysProducedWeapons(UObject* WorldContextObject, TArray<FFactorys>& GlobalFactorysData);
		void SelectOfferForCountry(UObject* WorldContextObject, FCountrys FirstCountry, FCountrys OpponentCountry, FRebellion RebellionsInSelectedCountry);
		void OfferTender();
